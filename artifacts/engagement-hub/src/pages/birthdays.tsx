...
import { Cake, CalendarHeart, Gift, Send } from "lucide-react";
            ...
            {todayBdays.map(b => (
              <motion.div variants={slideUp} key={b.id}>
                <Card className="bg-gradient-to-r from-pink-500 to-rose-500 text-white border-none shadow-lg overflow-hidden relative">
                  ...
                  <CardContent className="p-6 md:p-8 flex flex-col md:flex-row items-center gap-6 relative z-10 text-center md:text-left">
                    <Avatar className="w-24 h-24 border-4 border-white/20 shadow-xl">
                      <AvatarFallback className={cn("text-3xl text-white font-bold", b.employee.color)}>{b.employee.initials}</AvatarFallback>
                    </Avatar>
                    <div className="flex-1">
                      <p className="text-white/80 font-medium uppercase tracking-widest text-sm mb-1">Happy Birthday</p>
                      <h3 className="text-3xl font-bold mb-2">{b.employee.name}</h3>
                      <p className="text-white/90">{b.employee.role}</p>
                    </div>
                    {b.employee.id !== me?.id && (
                      <Button onClick={() => handleCelebrate(b.employee.name)} size="lg" className="bg-white text-pink-600 hover:bg-white/90 rounded-full w-full md:w-auto mt-4 md:mt-0 shadow-xl">
